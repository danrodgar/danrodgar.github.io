...
public class LinearNNSearch
  ...
  public Instances kNearestNeighbours(Instance target, int kNN) throws Exception {
    ...
    m_Indexes = new int[heap.size()+heap.noOfKthNearest()];
    int i=1; MyHeapElement h;
    while(heap.noOfKthNearest()>0) {
      h = heap.getKthNearest();
      m_Indexes[m_Indexes.length-i] = h.index;
      m_Distances[m_Indexes.length-i] = h.distance;
      i++;
    }
    while(heap.size()>0) {
      h = heap.get();
    ...
    for(int k=0; k<m_Indexes.length; k++) {
      neighbours.add(m_Instances.instance(m_Indexes[k]));
    }
   ...
   *            	or nearestNeighbours.
   */
  public int[] getIndexes() throws Exception {
    if(m_Indexes==null)
