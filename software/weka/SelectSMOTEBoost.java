 ...
 */
/*
 * SelectSMOTEBoost.java
 * 
 * Copyright (C) 2014 University of Alcala, Spain
 */
 ...
 * @version $Revision: 1 $
 */
public class SelectSMOTEBoost extends RandomizableIteratedSingleClassifierEnhancer
	...
	/** AttributeSelection - the attribute evaluator to use */
	private ASEvaluation m_AttributeSelection_Evaluator = new GainRatioAttributeEval();

	/** AttributeSelection - the search method if any */
	private ASSearch m_AttributeSelection_Search = new Ranker();
	 ...
	 * Constructor.
	 */
	public SelectSMOTEBoost() {

		m_Classifier = new weka.classifiers.trees.J48();
		attributeSelectionFilter = initAS();
	}
	...
	public Enumeration listOptions() {
		...
		newVector.addElement(new Option("\tSpecifies the random number seed\n"
				+ "\t(default 1)", "smoteS", 1, "-smoteS <num>"));

		newVector.addElement(new Option(
				"\tSpecifies percentage of SMOTE instances to create.\n"
						+ "\t(default 100.0)\n", "smoteP", 1, "-smoteP <percentage>"));
		...
		Enumeration enuAS = attributeSelectionFilter.listOptions();
		while (enuAS.hasMoreElements()) {
			newVector.addElement(enuAS.nextElement());
		}
	...
	public void setOptions(String[] options) throws Exception {
		...
		if (optionString.length() != 0) {
			...
			setAttributeSelection_Evaluator(ASEvaluation.forName(evalClassName, evalOptions));
		}

		if (m_AttributeSelection_Evaluator instanceof AttributeEvaluator) {
			setAttributeSelection_Search(new Ranker());
		}
		...
		if (optionString.length() != 0) {
			...
			setAttributeSelection_Search(ASSearch.forName(SearchClassName, SearchOptions));
		}
	...
	public String[] getOptions() {
		...
		options = attributeSelectionFilter.getOptions();
	 ...
	 *         explorer/experimenter gui
	 */
	public String AttributeSelection_EvaluatorTipText() {
		return "The index of the class value to which SMOTE should be applied. "
				+ "Use a value of 0 to auto-detect the non-empty minority class.";
	}
	
	/**
	 * @return the m_AttributeSelection_Evaluator
	 */
	public ASEvaluation getAttributeSelection_Evaluator() {
		return m_AttributeSelection_Evaluator;
	}

	/**
	 * @param m_AttributeSelection_Evaluator the m_AttributeSelection_Evaluator to set
	 */
	public void setAttributeSelection_Evaluator(ASEvaluation evaluator) {
		this.m_AttributeSelection_Evaluator = evaluator;
	 ...
	 *         explorer/experimenter gui
	 */
	public String AttributeSelection_SearchTipText() {
		return "The index of the class value to which SMOTE should be applied. "
				+ "Use a value of 0 to auto-detect the non-empty minority class.";
	}
	
	/**
	 * @return the m_AttributeSelection_Search
	 */
	public ASSearch getAttributeSelection_Search() {
		return m_AttributeSelection_Search;
	}

	/**
	 * @param m_AttributeSelection_Search the m_AttributeSelection_Search to set
	 */
	public void setAttributeSelection_Search(ASSearch search) {
		this.m_AttributeSelection_Search = search;
	...
	public AttributeSelection initAS() {
		
		AttributeSelection as = new AttributeSelection();
		
		as.setEvaluator(m_AttributeSelection_Evaluator);
		as.setSearch(m_AttributeSelection_Search);
	...
	protected void buildClassifierUsingResampling(Instances data)
		...
		if (m_Debug) {
			...
			for (int i = 0; i < training.size(); i++)
				System.err.print(Math.rint(training.get(i).weight() * 100)
						/ 100 + ";");
			System.err.println();
		}
		...
		for (m_NumIterationsPerformed = 0; m_NumIterationsPerformed < m_Classifiers.length; m_NumIterationsPerformed++) {
			...
			if (m_Debug) {
				System.err.println("iteration: "
						+ (m_NumIterationsPerformed + 1)
						+ " trainData sumOfWeights before SMOTE: "
						+ training.sumOfWeights() + " Nº instances: "
						+ training.size());
			}
			...
			// AttributeSelection
			// Initializing AttributeSelection
			attributeSelectionFilter.setInputFormat(trainData);
			trainData = Filter.useFilter(trainData, attributeSelectionFilter);

			if (m_Debug) {
				System.err.println("\tattributes: " + trainData.numAttributes() + "\n");
				System.err.println("\tinstances: " + trainData.numInstances() + "\n");
			}
			
			if (m_Debug) {
				System.err.println("iteration: "
						+ (m_NumIterationsPerformed + 1)
						+ " trainData sumOfWeights after SMOTE: "
						+ trainData.sumOfWeights() + " Nº instances: "
						+ trainData.size());
			}
	...
	public static void main(String[] argv) {
		runClassifier(new SelectSMOTEBoost(), argv);
	}
}
