 ...
 * AllkNN.java
 * 
 * Copyright (C) 2008 Daniel Rodriguez, Univ of Alcala
 */
...
public class AllkNN extends Filter implements SupervisedFilter, OptionHandler,
	...
	public boolean batchFinished() throws Exception {
		...
		if (!m_FirstBatchDone) {
			// Do AllkNN, and clear the input instances.
			doAllkNN();
		}
		flushInput();
	...
	/**
	 * The procedure implementing the AllkNN algorithm. The output instances are
	 * pushed onto the output queue for collection.
	 * 
	 * @throws Exception
	 *             if provided options cannot be executed on input instances
	 */
	protected void doAllkNN() throws Exception {
		...
		// AllkNN - For all instances, an instance is kept if its class and 
	    ...
	    for (int i = 0; i < sampleSize; i++) {
	        ...
	        // 2. For all k nearest neighbours
	     	int currentInstClassValue = (int)target.classValue();
	     	boolean bRemove = false;
	     	//
	        for (int n=1; n<=getNumberOfNN()  && !bRemove; n++) {
		        // Get k neighbours of current instance (target)
		        Instances neighbours = NB.kNearestNeighbours(target, n);        	
	            // 2.1. Initialize counters of classes
	            for(int nIdx=0; nIdx < nClasses; nIdx++){
	                nNerarestNeighborsClasses[nIdx] = 0;
	            }         
	            // 3.2 Get the n (1..k) number of neighbours' classes in the array
		     	for (int j = 0; j < neighbours.numInstances(); j++) {
	                nNerarestNeighborsClasses[(int) neighbours.get(j).classValue()]++;
		   	    }	     	
...
//	     	int maxIndex = -1;  
//	     	for(int nIdx = 0; nIdx < nClasses; nIdx++) {
//	     	      if(nNerarestNeighborsClasses[nIdx] > max) {
//	     	         max = nNerarestNeighborsClasses[nIdx];
//	     	         maxIndex = nIdx;
//	     	      }
//	     	}
//	     		     		        
//	        //if current instance belongs to the mayority class then instance is pushed into the queue
//	        int currentInstClassValue = (int)target.classValue();
//	     	if (maxIndex == currentInstClassValue) {
//	     		push((Instance) target.copy());
//	     	}
//	     	//else System.out.println("Through away this one!"); 
	     }
	}
